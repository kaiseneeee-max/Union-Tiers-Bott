...
function getGuildData(guildId) {
  if (!database.guilds[guildId]) {
    database.guilds[guildId] = {
      ...
      // Ticket notification roles
      notifyRoles: [],

      // /message roles
      messageRoles: [],

      // /result + /highresults roles
      resultRoles: [],

      // Ticket access + close roles
      ticketRoles: [],
...
function canAccessTickets(member, guildData) {
  if (!member) {
    return false;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Select the kit you want to test.\n\n` +
        ...
        `🧪 **Configured testers can submit test results.**`
    )
    .setColor(0xffc107);
...
function getAllTicketStaffRoles(guildData) {
  return [
    ...new Set([
...
async function getOrCreateTicketCategory(
  ...
  const staffRoles =
    getAllTicketStaffRoles(guildData);
...
async function updateCategoryPermissions(
  ...
  for (const ticket of existingTickets.values()) {
    for (const roleId of staffRoles) {
...
client.once(
  ...
  async () => {
    ...
    try {
      ...
      console.log(
        "✅ /result = testers + result roles"
      );

      console.log(
        "✅ /highresults = testers + result roles"
      );
    ...
    try {
      ...
      if (
        ...
        return interaction.reply({
          content:
            ...
            `🎟️ **Ticket roles:** ${guildData.ticketRoles.length}\n` +
            `🏆 **Result roles:** ${guildData.resultRoles.length}\n` +
            `🔔 **Notification roles:** ${notifyRoles.length}\n` +
            `📁 **Ticket category:** ${category.name}\n\n` +
            `🏆 **/result:** Testers + Results roles\n` +
            `👑 **/highresults:** Testers + Results roles\n` +
            `🎟️ **Ticket roles:** View + close tickets`,
      ...
      if (
        ...
        if (type === "results") {
          ...
          return interaction.reply({
            content:
              `✅ Added ${roles.length} role(s) to **Results** permission.\n\n` +
              `🏆 They can now use **/result** and **/highresults**.`,
        ...
        if (type === "ticket") {
          ...
          // Update category and existing tickets
          await updateCategoryPermissions(
            guild,
            guildData,
            category
