...
// High results can only start from LT3
...
function canUseResults(member, guildData) {
  ...
  // Testers can use results
  if (isTester(member, guildData)) {
    return true;
  }

  // /addrole results roles can use BOTH result commands
...
function canUseTickets(member, guildData) {
  ...
  // Testers automatically have ticket permissions
  if (isTester(member, guildData)) {
    return true;
  }

  // Roles added using /addrole ticket
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Select the kit you want to test.\n\n` +
        ...
        `🧪 **Only authorized staff can manage tickets.**`
    )
    .setColor(0xffc107);
...
function buildResultCommand({
  ...
  command.addStringOption((option) =>
    option
      .setName("skin")
      .setDescription(
        "Optional skin image URL. Leave empty for Steve."
      )
      .setRequired(false)
  );
...
client.once(
  ...
  async () => {
    ...
    try {
      ...
      console.log(
        "✅ /addrole now supports Message, Results and Ticket."
      );
    ...
    try {
      ...
      if (
        ...
        return interaction.update({
          content:
            ...
            `🧪 Authorized testers and ticket staff can access it.\n\n` +
            `🎫 ${ticket}`,
      ...
      if (
        ...
        if (
          ...
          // Player can still view their own ticket info
          const parts =
            (
